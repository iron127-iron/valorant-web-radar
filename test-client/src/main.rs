    ...
    loop {
        match connect("ws://localhost:27017") {
            Ok((mut socket, _response)) => loop {
                ...
                let _played_rounds = rng.random_range(0..30);
                let mut _max_rounds = 24;
                let mut _spike_planted = rng.random_range(0..2);
                ...
                if _spike_planted == 1 {
                    _spike_x.push(json::from(rng.random_range(0.0..1000.0)));
                    _spike_y.push(json::from(rng.random_range(0.0..1000.0)));
                    _spike_time.push(json::from(rng.random_range(0.0..45.0)));
                    _round_time.push(_spike_time[0].clone());
                } else {
                    _spike_x.push(json::from(0));
                    _spike_y.push(json::from(0));
                    _spike_time.push(json::from(0));
                    _round_time.push(json::from(rng.random_range(0.0..150.0)));
                }
                ...
                for _i in 0.._max_rounds {
                    if _i < _played_rounds {
                        _round_win_status.push(json::from(rng.random_range(0..2)));
                    } else {
                ...
                for _i in 0..10 {
                    _id.push(json::from(rng.random_range(0..22)));
                    _x.push(json::from(rng.random_range(0.0..1000.0)));
                    _y.push(json::from(rng.random_range(0.0..1000.0)));
                    _health.push(json::from(rng.random_range(0..101)));
                    match _i {
                        0..=4 => _team.push(json::from(0)),
                        5..=10 => _team.push(json::from(1)),
                        _ => (),
                    }
                    _dormant.push(json::from(rng.random_range(0..2)));
                    _rotation.push(json::from(rng.random_range(0.0..360.0)));
                    _scoped.push(json::from(rng.random_range(0..2)));
                    _weapon.push(json::from(rng.random_range(0..100)));
                    _kill.push(json::from(rng.random_range(0..30)));
                    _death.push(json::from(rng.random_range(0..30)));
                    _assist.push(json::from(rng.random_range(0..30)));
                    _acs.push(json::from(rng.random_range(0..400)));
                    _shield.push(json::from(rng.random_range(0..50)));
                    _credits.push(json::from(rng.random_range(0..16000)));
                    _defusing.push(json::from(0));
                    if _spike_planted == 1 && _defusing_spike == 0 && _i > 4 {
                        _defuse_time.push(json::from(rng.random_range(0.0..8.0)));
                        _defusing.pop();
